...
class CocoDetection(torchvision.datasets.CocoDetection):
    def __init__(self, img_folder, processor):
        ann_file = os.path.join(img_folder, "cnr2.json")
...
def show(prediction, image, image_id):
    """ Plot an image with bounding boxes """
    fig, ax = plt.subplots()
    ax.imshow(image)
    index = 0
    for bbox in prediction['boxes'].cpu():
        ...
        index = index + 1
    #plt.show()
    plt.savefig('output_cnr/{}.jpg'.format(image_id))
    plt.close(fig)
...
for idx, batch in enumerate(tqdm(val_dataloader)):
    ...
    for index, (image_id, prediction) in enumerate(predictions.items()):
        ...
        show(prediction, val_dataset._load_image(image_id), image_id)
